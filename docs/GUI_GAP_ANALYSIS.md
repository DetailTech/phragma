...
record current implementation coverage under each gap where applicable. The
WebUI is no longer a research-only baseline: the current canonical set in
`make webui-enterprise-smoke` contains 19 registered routes, including `/proxy`
and `/compliance`. The configured Makefile path list includes `/dashboard` as
an alias for `/`, so the path list has one more entry than the canonical route
count. Existing remote smoke results remain tied to the source snapshot and
route inventory they recorded. Tablet/mobile sweeps remain diagnostic unless
the release contract is deliberately changed, and durable release evidence
must still be regenerated after the source-control state is accepted.
...
| Local trust, auth, and audit | Settings covers local users, roles, OIDC/SAML rollout packets, parsed OIDC/SAML field-evidence artifact inventory, session posture, functional step-up controls, advisory RBAC previews, access guardrails, and network/host-input/telemetry posture; Changes owns audit and apply-preflight review | Yes: `/settings`, `/changes` | None identified for the bounded OIDC/SAML field-run evidence workflow; `status=passed` remains a release-artifact sentinel, not external certification | External OIDC/SAML provider certification, production IdP/MFA reauth, SAML certificate rollover, secret custody, time-source, audit backpressure, signed provider evidence, signed audit export |
...
| Fleet, HA, and system posture | Dashboard exposes the global runtime/engine rollup; Fleet exposes HA, local-appliance, release-summary, template, diff/drift, positive-evidence orchestration, bounded apply-plan, and retained local result posture; Changes owns runtime preflight action items | Yes: `/`, `/fleet`, `/changes` | None identified for the bounded local template apply/result-custody workflow | Signed fleet/HA evidence, peer RPC/fanout, peer attestation, HA fencing, VIP movement, traffic failover, stale-evidence trend retention, and production custody for scrubbed runtime packets |
...
- Current Traffic, Threats, audit pivots, and Investigation surfaces support
  ...
  alert, capture, App-ID, NAT/route, Settings telemetry/OIDC, Fleet HA, Net/VPN,
  object-reference, audit, and Intel content-package evidence. Deeper
...
- `#/logs` now adds a first-class System Logs workbench backed by
  ...
  drawers, Investigation pinning, and pivots to Changes audit, Traffic, Threats,
  and Troubleshoot. Log drawers now derive flow, tuple, App-ID, and
...
- Guided Setup stages a two-zone baseline through the normal candidate path and now includes a persistent first-run checklist for interface boundaries, TLS/auth/OIDC admin access, host-input exposure, content/update package posture, outbound/NAT posture, inspection mode, host tuning evidence, and candidate review. Checklist actions link to Settings access and system panels, Intel, Changes, and API/CLI context instead of committing directly.
