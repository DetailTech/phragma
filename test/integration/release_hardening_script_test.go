...
func TestReleaseDeployHardeningCheckAcceptsPackagedArtifacts(t *testing.T) {
	...
	for _, want := range []string{
		...
		"ok: bounded rootless prebuilt-pair probe is explicit",
		"ok: prebuilt controld uses its supported version flag",
		"ok: prebuilt ngfwctl uses its supported version subcommand",
		"ok: installer mutations require root",
