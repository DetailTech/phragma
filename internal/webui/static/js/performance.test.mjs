...
assert.doesNotMatch(performanceViewSource, /#\/readiness\?/);
assert.doesNotMatch(performanceViewSource, /api\.releaseAcceptanceStatus\(\)/);
...

{
  ...
  assert.equal(packet.summary.releaseEvidenceHref, undefined);
  assert.equal(packet.summary.supportBundleHref, undefined);
  ...
  assert.equal(packet.artifacts.operatorDecision.releaseEvidenceHref, undefined);
...

{
  ...
  assert.equal(decision.releaseEvidenceHref, "");
  assert.equal(decision.releaseAcceptanceHref, "");
  assert.equal(decision.supportBundleHref, "");
