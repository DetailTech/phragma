...
assert.match(diagnosticConsole, /title: "Open readiness workbench", "aria-label": "Open readiness workbench", dataset: \{ action: "readiness", sharedControl: "diagnostic-readiness" \}/);
...
assert.match(css, /\.search-btn \{[^}]*border-radius: var\(--radius\);[^}]*white-space: nowrap;/);
assert.doesNotMatch(css, /\.search-btn \{[^}]*border-radius: 999px;/);
assert.match(css, /\.chip \{[^}]*border-radius: var\(--radius\);/);
assert.doesNotMatch(css, /\.chip \{[^}]*border-radius: 999px;/);
